...
import { z } from "zod";
import { finalizeWorkshopPrompt, sendWorkshopMessage } from "@/lib/server/prompt-workshop-service";
...
export async function PATCH(
  ...
  try {
    const { projectId } = await context.params;
    return NextResponse.json(finalizeWorkshopPrompt(projectId));
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to finalize workshop prompt.",
      },
      { status: 400 },
    );
  }
